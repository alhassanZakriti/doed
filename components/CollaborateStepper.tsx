...
export function CollaborateStepper({ steps }: { steps: CollaborateStep[] }) {
  ...
  return (
    <ol className="relative mt-10 flex flex-col gap-6 overflow-visible px-2 md:mt-14 md:flex-row md:items-start md:justify-between md:gap-3 md:pt-10 md:pb-4">
      <span
        className="absolute bottom-6 left-8 top-6 w-px bg-foreground/20 md:hidden"
        aria-hidden="true"
      />
      <span className="absolute left-10 right-10 top-[4rem] hidden h-px bg-foreground/20 md:block" aria-hidden="true" />
        ...
        return (
          ...
          >
            <button
              ...
              className="flex w-full items-start gap-4 text-left focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-orange-500 md:flex-col md:items-center md:text-center"
            >
              <span className={`relative flex h-12 w-12 shrink-0 items-center justify-center overflow-visible ${isActive ? "" : "is-paused"}`}>
                ...
                <span className="pulse-ring pulse-ring-orange" />
                <span
                  className={`relative z-1 flex h-12 w-12 items-center justify-center rounded-full transition-colors duration-500 ease-out ${
                    isActive ? "bg-orange-500 text-lg font-extrabold text-on-accent" : "bg-navy-900 text-base font-bold text-white dark:ring-1 dark:ring-white/25"
                  }`}
                >
                  {step.n}
                </span>
              </span>
              <span className="mt-0 flex min-h-0 w-full max-w-none flex-col items-start md:mt-3 md:min-h-[6.5rem] md:max-w-[200px] md:items-center">
                <span
